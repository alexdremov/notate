...
class CanvasRepositoryJitTest {
    ...
    private fun createDummyStroke(id: Int, xOff: Float = 0f, yOff: Float = 0f): Stroke {
        val points = listOf(
            com.onyx.android.sdk.data.note.TouchPoint(xOff, yOff, 0.5f, 1f, 0, 0, System.currentTimeMillis()),
            com.onyx.android.sdk.data.note.TouchPoint(xOff + 10f, yOff + 10f, 0.5f, 1f, 0, 0, System.currentTimeMillis())
        )
    ...
    private fun createHeavyCanvasZip(path: String, regionCount: Int): File {
        ...
        val metaBytes = kotlinx.serialization.protobuf.ProtoBuf.encodeToByteArray(CanvasData.serializer(), metadata)
    ...
    @Test
    fun `test JIT open does not extract all files immediately`() = runBlocking {
    ...
    @Test
    fun `test on-demand extraction works before background unzip completes`() = runBlocking {
    ...
    @Test
    fun `test save waits for initialization`() = runBlocking {
    ...
    @Test
    fun `test corruption recovery - missing regions in zip`() = runBlocking {
        ...
        File(tempDir, "manifest.bin").writeBytes(
            kotlinx.serialization.protobuf.ProtoBuf.encodeToByteArray(CanvasData.serializer(), metadata)
        )
    ...
    @Test
    fun `test corruption recovery - corrupted region file in zip`() = runBlocking {
    ...
    @Test
    fun `test external modification detection`() = runBlocking {
