...
class DrawingViewModelTest {
    ...
    @Test
    fun `saveCanvasSession should call repository save`() =
        runTest {
            val path = "test/path.notate"
            val session = mockk<CanvasSession>(relaxed = true)
            val metadata = CanvasData(version = 3)
    ...
    @Test
    fun `closeSession should clear currentSession and call saveAndClose`() =
        runTest {
            val path = "test/path.notate"
            val session = mockk<CanvasSession>(relaxed = true)
    ...
    @Test
    fun `addToolbarItem should update toolbarItems flow`() =
        runTest {
            val item =
                com.alexdremov.notate.model.ToolbarItem.Pen(
                    com.alexdremov.notate.model
                        .PenTool(id = "test", name = "Test", type = com.alexdremov.notate.model.ToolType.PEN),
                )
