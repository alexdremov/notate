...
class OnyxCanvasView
    ...
    constructor(
        ...
        private val viewportInteractor = ViewportInteractor(
            context, 
            matrix, 
            invalidateCallback = { 
                minimapDrawer.show()
        ...
        init {
            ...
            canvasController.setViewportController(object : com.alexdremov.notate.controller.ViewportController {
        ...
        private fun drawContent() {
            ...
            try {
                ...
                if (CanvasConfig.DEBUG_SHOW_RAM_USAGE) {
                    ...
                    val debugPaint = android.graphics.Paint().apply { color = Color.RED; textSize = 40f }
        ...
        fun scrollByOffset(dx: Float, dy: Float) {
        ...
        fun setEraser(tool: PenTool) { penInputHandler.setEraserTool(tool) }
        fun setCursorView(view: CursorView) { penInputHandler.setCursorView(view) }
        fun setBackgroundStyle(style: com.alexdremov.notate.model.BackgroundStyle) { canvasModel.setBackground(style); invalidateCanvas(); onContentChanged?.invoke() }
        fun getBackgroundStyle() = canvasModel.backgroundStyle
        
        fun setDrawingEnabled(enabled: Boolean) {
            if (enabled) setupTouchHelper() else {
                touchHelper?.setRawDrawingEnabled(false)
                touchHelper?.closeRawDrawing()
                EpdController.setScreenHandWritingPenState(this, EpdPenManager.PEN_PAUSE)
            }
        ...
        fun setExclusionRects(rects: List<Rect>) {
            exclusionRects.clear(); exclusionRects.addAll(rects)
            touchHelper?.let { val limit = Rect(); getLocalVisibleRect(limit); it.setLimitRect(limit, exclusionRects) }
        }
        
        fun clear() {
            canvasModel.clear(); canvasRenderer.clearTiles(); minimapDrawer.setDirty(); drawContent()
            touchHelper?.setRawDrawingEnabled(false); touchHelper?.setRawDrawingEnabled(true); onContentChanged?.invoke()
        }
        
        fun undo() { canvasModel.undo()?.let { canvasRenderer.invalidateTiles(it) }; refreshAfterEdit(); onContentChanged?.invoke() }
        fun redo() { canvasModel.redo()?.let { canvasRenderer.invalidateTiles(it) }; refreshAfterEdit(); onContentChanged?.invoke() }
            ...
            if (sm.hasSelection()) {
                ...
                if (!selectionInteractor.isInteracting()) {
                    ...
                    if (!bounds.isEmpty && bounds.width() > 1f && bounds.height() > 1f) {
                        // Pass View Matrix to map World Bounds -> Screen
                        actionPopup?.show(this, bounds, matrix)
                    } else { actionPopup?.dismiss() }
                }
            } else { actionPopup?.dismiss() }
        }
        
        fun dismissActionPopup() { actionPopup?.dismiss() }
        ...
        private fun setupTouchHelper() {
            ...
            com.alexdremov.notate.util.OnyxSystemHelper.ignoreSystemSideButton(this)
            val limit = Rect(); getLocalVisibleRect(limit)
            touchHelper?.apply {
